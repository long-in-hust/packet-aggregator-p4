...
def decodeMac(encoded_mac_addr):
    # Python3: encoded_mac_addr is bytes, format each byte as two hex digits
...
def encodeNum(number, bitwidth):
    ...
    # Use Python 3's to_bytes to construct the big-endian byte string
    return number.to_bytes(byte_len, byteorder='big')

def decodeNum(encoded_number):
    # Use Python 3's int.from_bytes to decode big-endian bytes to int
